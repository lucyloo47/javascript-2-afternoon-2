...
function looper (family){
  family.forEach (function(val, i, family){
    alert (family[i])
  })
}
...
function evenFinder(nums){
  var nums = nums.filter (function(val){
    return val % 2 === 0;
  });
  return nums;
}
...
function divider (numbersArray){
  var newArray = [];
  newArray[0] = numbersArray.filter (function(val){
    return val % 2 === 0;
  });
  newArray[1] = numbersArray.filter (function(val){
    return val % 2 != 0;
  });
  return newArray;
}
...
devMountainEmployees.forEach(function(val,i,arr){
...
users.forEach (function(val, i, arr){
  if (users[i].val = "tylermcginnis33@gmail.com"){
    users.splice ([i],1);
  }
})
